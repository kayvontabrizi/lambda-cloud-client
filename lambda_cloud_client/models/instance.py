...
class Instance(BaseModel):
    ...
    ] = Field(default=None, description="User-provided name for the instance")
    ip: Optional[StrictStr] = Field(
        default=None, description="IPv4 address of the instance"
    )
    private_ip: Optional[StrictStr] = Field(
        default=None, description="Private IPv4 address of the instance"
    )
    status: StrictStr = Field(description="The current status of the instance")
    ssh_key_names: List[Annotated[str, Field(strict=True, max_length=64)]] = Field(
        description="Names of the SSH keys allowed to access the instance"
    )
    file_system_names: List[StrictStr] = Field(
    ...
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(
            ["active", "booting", "unhealthy", "terminating", "terminated"]
        ):
            raise ValueError(
                "must be one of enum values ('active', 'booting', 'unhealthy', 'terminating', 'terminated')"
            )
        return value
