...
class LaunchInstanceRequest(BaseModel):
    ...
    file_system_names: Optional[
        Annotated[List[StrictStr], Field(max_length=1)]
    ] = Field(
        default=None,
        description="Names of the file systems to attach to the instances. Currently, only one (if any) file system may be specified.",
    )
    quantity: Optional[Annotated[int, Field(le=1, strict=True, ge=1)]] = Field(
        default=1, description="Number of instances to launch"
    )
    name: Optional[
